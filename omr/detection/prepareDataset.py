...
def write_yaml_dataset(
    path, train_path, val_path, label_df=None, filename="deep_scores.yaml"
):
    label_yaml = "names:\n"
...
write_yaml_dataset(
    ...
    test_dir_relative,
    labels,
    filename=dataset_path / "deep_scores.yaml",
)
...
def extract_info(comment):
    ...
    return [
        int(duration.group(1)) if duration else None,
...
train_obboxs["label"] = (
    train_obboxs["cat_id"]
    .apply(map_cat_ids_to_classes)
    .apply(clean_labels)
    .apply(select_highest_precedence)
)
test_obboxs["label"] = (
    test_obboxs["cat_id"]
    .apply(map_cat_ids_to_classes)
...
train_obboxs[["duration", "rel_position"]] = (
    train_obboxs["comments"].apply(extract_info).tolist()
)
test_obboxs[["duration", "rel_position"]] = (
    test_obboxs["comments"].apply(extract_info).tolist()
)
...
train_data = pd.merge(train_obboxs, train_images, on="img_id", how="inner").drop(
    "ann_ids", axis=1
)
test_data = pd.merge(test_obboxs, test_images, on="img_id", how="inner").drop(
    "ann_ids", axis=1
)
...
filename_to_dimensions = dict(
    zip(train_images["filename"], zip(train_images["width"], train_images["height"]))
)
measures_df["width"] = measures_df["filename"].map(
    lambda x: filename_to_dimensions.get(x, (np.nan, np.nan))[0]
)
measures_df["height"] = measures_df["filename"].map(
...
filename_to_dimensions = dict(
    zip(test_images["filename"], zip(test_images["width"], test_images["height"]))
)
measures_df_test["width"] = measures_df_test["filename"].map(
    lambda x: filename_to_dimensions.get(x, (np.nan, np.nan))[0]
)
measures_df_test["height"] = measures_df_test["filename"].map(
    lambda x: filename_to_dimensions.get(x, (np.nan, np.nan))[1]
)
...
def corners_to_yolo(bbox, img_width, img_height):
    bbox = [
        max(min(bbox[i], img_width - 1 if i % 2 == 0 else img_height - 1), 0)
...
missing_annotations = measures_df_test[
    measures_df_test["filename"].isin(test_data["filename"])
]
...
df_agg = (
    train_data.groupby("filename")
    .agg({"yolo_bbox": lambda x: list(x), "label": lambda x: list(x)})
    .reset_index()
)
df_test_agg = (
    test_data.groupby("filename")
    .agg({"yolo_bbox": lambda x: list(x), "label": lambda x: list(x)})
    .reset_index()
)
df_agg["yolo_bbox"] = df_agg["yolo_bbox"].apply(
    ...
    for _, row in df.iterrows():
        ...
        with open(text_file_path, "w") as text_file:
            for bbox, class_label in zip(yolo_bbox, label):
                text_file.write(
                    f"{class_label} {bbox[0]} {bbox[1]} {bbox[2]} {bbox[3]}\n"
                )
